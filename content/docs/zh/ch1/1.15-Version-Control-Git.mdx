...
> [!DETAILS-AI] 本节 AI 摘要
> 本节从工作区、暂存区和提交历史讲起，介绍 Git 的日常提交、分支协作、冲突处理与撤销方法。重点是先判断改动位于哪里，再选择不会误伤其他内容的命令。
...
> [!WARNING] 提交邮箱与账户关联
> 如果希望提交关联到 GitHub 账户，应使用 GitHub 账户中已验证的邮箱（推荐）或 GitHub 提供的 `noreply` 邮箱。提交身份与登录凭据两者并不同，修改前可用 `git config --global --get user.email` 检查本地配置的邮箱。
...
如果当前分支是被合并分支的直接祖先，Git 可能执行 **fast-forward**：只移动当前分支引用，不创建新的合并提交。如果两条分支在共同祖先之后都产生了提交，Git 通常需要基于共同祖先执行 **three-way merge**，并可能创建一个合并提交；无法自动决定的文件会产生冲突（见下节）。是否强制创建合并提交，可按项目约定使用 `--no-ff`。
...
**three-way merge：两条分支在共同祖先之后都产生了提交**
...
两条分支在共同祖先 B 之后各自前进，Git 基于共同祖先做三方合并，产生合并提交 M。
...
通常 `HEAD` 指向当前分支；执行 `git checkout a1b2c3d` 或 `git switch --detach a1b2c3d` 后，`HEAD` 会直接指向某个提交，处于 **detached HEAD** 状态。此时可以查看历史或临时测试，但直接创建的新提交不属于任何分支，离开前应创建分支保存它：
...
> `merge` 保留分支结构并产生合并提交，历史如实记录分支何时汇合；`rebase` 把提交重新应用到目标分支顶端，得到线性历史，但会生成新的提交哈希。
>
> 判断依据主要是历史是否已被共享：已经推送到远程或多人协作的分支，优先使用 `merge` 而不是改写历史；尚未推送的本地分支，可以根据项目约定选择，想要整洁的线性历史时用 `rebase`。冲突处理上 `merge` 一次解决，`rebase` 可能需要在重放的每个提交上逐个解决。具体取舍仍以项目约定为准。
...
| 期数 | 主题 | 对应本节章节 |
| :--- | :--- | :--- |
| 01 | Git 设计理念与版本控制历史 | 一、为啥要用 Git？；二、Git 的核心概念 |
| 02 | Git 安装与配置、三层配置文件 | 三、初始配置 |
| 03 | 仓库基本使用、提交、文件更改与暂存 | 四、基本工作流；六、暂存的精细控制 |
...
| 13-A | GitHub 账户授权、分支保护、PR、Issue、Releases | [1.16 GitHub 与 CNB](/zh/docs/ch1/1.16-GitHub-CNB) |
| 13-B | 参与大型开源项目 | [1.16 GitHub 与 CNB](/zh/docs/ch1/1.16-GitHub-CNB) |
