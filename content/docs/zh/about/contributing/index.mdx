...
| 命令 | 用途 | 是否写入工作区 |
| :--- | :--- | :---: |
| `bun dev` | 生成 Mermaid 资源并启动 Turbopack 开发服务器 | 是 |
| `bun lint` | 运行 Biome Lint | 否 |
| `bun typecheck` | 生成路由与内容类型，校验 MDX、Frontmatter 和 TypeScript | 是 |
| `bun run generate:mermaid` | 重新生成 Mermaid 静态 SVG 与资源映射 | 是 |
| `bun run build` | 生成 Mermaid 资源并执行生产静态构建 | 是 |
...
`author`、`contributor` 和 `contributors` 支持单个字符串或字符串数组；`draft` 与 `todoProgress` 默认均为 `false`。不要写入 Schema 未声明的自定义字段。
...
完整写法与渲染示例见 [语法与组件参考](./syntax-example)。
...
| Mermaid 内容或数量变化 | `bun run generate:mermaid`，并检查生成资源 |
