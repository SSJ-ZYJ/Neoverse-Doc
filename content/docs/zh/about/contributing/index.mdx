...
浏览器打开 `http://localhost:3001` 即可预览。不要在没有讨论的情况下新增、删除、升级或替换依赖；当前版本以 `package.json` 为准。
...
| `bun run report:content` | 输出 Content IR Metadata Coverage 与 Content Health；可追加 `-- --verbose` 或 `-- --taxonomy` | 否 |
| `bun run check:content` | 校验 Schema、生命周期、关系、翻译漂移与 Mermaid 资产；Coverage 不足不会失败 | 是 |
...
`id` 是必填的稳定内容身份：新建页面时取与最终路径一致的值（如 `ch1/your-page-name`），中英文版本必须声明相同 `id`；此后移动文件或调整 URL 都不要改它。模板中的 `type`、`topics`、`tracks` 与 `difficulty` 是新页面的最小推荐 Metadata，但仍然是可渐进补齐的可选字段。`estimatedMinutes`、`prerequisites`、`related`、`lastReviewed`、`author`、`contributor`、`contributors`、`status` 与 `todoProgress` 也都不是模板必填项；`status` 默认是 `stable`。`lastReviewed` 只填写人工完成技术复核的日期；译文完成同步确认后再填写对应的 `reviewedRevision`。不要写入 Schema 未声明的自定义字段。

运行 `bun run report:content` 查看当前覆盖率与维护摘要；缺失 Metadata 只产生 authoring warning，不会阻断 CI。需要完整逐文件诊断时使用 `bun run report:content -- --verbose`，需要合法 Type、Topic、Track 或 Difficulty ID 时使用 `bun run report:content -- --taxonomy`。这些列表均由 Taxonomy Registry 自动生成，不要在文档中另写一份。
