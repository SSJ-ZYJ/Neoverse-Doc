...
<Files>
  <Folder name="Neoverse-Doc" defaultOpen>
    ...
    <Folder name="scripts" disabled title="构建期生成脚本" />
    <Folder name="src" defaultOpen>
      ...
      <Folder name="generated" disabled title="生成的资源映射" />
      <Folder name="lib" disabled title="配置、内容源与工具函数" />
    </Folder>
...
常用类型包括 `feat`、`fix`、`docs`、`style`、`refactor`、`test`、`chore`、`ci` 与 `revert`。例如：
...
摘要保持简短；需要补充背景时，在空行后添加正文。PR 标题可以沿用相同格式。
...
感谢你愿意让 Neoverse-Docs 变得更准确、更易读。遇到不确定的问题时，可以先在 [GitHub Issues](https://github.com/SSJ-ZYJ/Neoverse-Doc/issues) 中讨论。
