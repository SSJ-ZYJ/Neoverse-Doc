...
│   ├── compositions/compositions.css # 页面头部、区段、网格、空状态与 Metadata Chip
...
Task、Mermaid、Transition、Community、Search Spotlight 与 Interaction 样式跟随其所有者；稳定复用的产品页面组合位于 `src/ui/compositions/compositions.css`，页面特有规则继续留在对应 Feature，其他页面级布局和少量全局集成规则继续留在 `src/styles/`。视觉调整优先替换 Token 或已有表面，不继续叠加第二套玻璃、Glow 或阴影体系。

页面组合只统一信息层级与节奏，不拥有页面数据或路由。`Page Header`、`Page Section`、`Content Grid` 与 `Empty State` 通过 CSS 组合类复用；`Metadata Chip` 用有限的语义变体表达 Track、Topic、Type、Difficulty 与生命周期 Status，文案仍由字典或 Taxonomy Registry 提供。
...
| `src/ui/compositions/compositions.css` | 页面组合与统一 Metadata Chip |
