...
`source.config.ts` 在 Fumadocs `pageSchema` 上扩展项目字段：
...
`source.config.ts` 按以下顺序组装 MDX 插件，每个插件只承担单一职责：
...
| `src/lib/remark-code-title.ts` | 路径标题提取 |
| `src/lib/remark-collapsible-alert.ts` | 语义折叠语法 |
| `src/lib/remark-github-alert.ts` | GitHub Alert 语法 |
| `src/lib/remark-lang-alias.ts` | 非内置语言标识符重写 |
| `src/lib/remark-long-code-block.ts` | 超长代码块降级 |
| `src/lib/transformer-meta-title.ts` | 标题属性转换与图标恢复 |
| `src/components/mdx/custom-codeblock.tsx` | 代码块服务端外壳 |
| `src/components/mdx/code-tabs.tsx` | Tabs 与滑动下划线 |
| `src/components/mdx/task-list-item.tsx` | 服务端任务识别 |
| `src/components/mdx/interactive-task-list-item.tsx` | 本地任务状态 |
| `src/components/mdx/task-list-progress.tsx` | 页面进度派生 |
