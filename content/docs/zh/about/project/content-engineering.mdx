...
枚举、格式与数值约束由 zod 在编译期强制；跨页引用（ID 唯一、引用存在、自引用防护、翻译配对路径对称）由 `scripts/check-content.ts` 基于编译后的内容清单校验（`bun run check:content`，已挂入 `prebuild`）。引用采用宽松 locale 语义——目标在任意语言下存在即合法，翻译缺口不破坏内容关系。
