...
`src/app/api/search/route.ts` 使用 Fumadocs `createFromSource()` 从内容源生成搜索数据，并设置 `dynamic = 'force-static'`。生产构建会把这个 Route Handler 输出为静态端点，不需要请求时扫描 Markdown。
...
每条索引记录包含页面 URL、标题、描述、结构化正文与一级 slug。一级 slug 同时写入 Fumadocs `tag`，因此 Chapter 筛选直接复用内容路径，不需要维护第二份章节映射。

## 二、中英混合与拼音检索

默认英文分词无法可靠处理连续中文，简单按字符切分又会损害英文单词和短语。`src/lib/search-tokenizer.ts` 组合了两类行为：
...
`src/lib/search-client.ts` 的 `withEnhancedSearch()` 包装原始搜索客户端，按以下流水线处理结果：
...
`src/components/search.tsx` 使用 Fumadocs `useDocsSearch` 和静态客户端：
...
`src/components/search-spotlight.tsx` 实现搜索结果跳转后的短时全局聚光灯：
...
`src/lib/search-spotlight.ts` 提供聚光灯参数处理：
...
`src/lib/docs-reading-restore.ts` 处理页面刷新后的滚动恢复：
...
- `src/lib/layout.shared.tsx` 扩展 Fumadocs 内置 UI 翻译（包括 `Ask AI`、`Close Sidebar`、`Layout Tab` 等 16.13.x 新增 key）；
...
| `src/lib/search-tokenizer.ts` | 中文、英文与拼音混合分词 |
| `src/lib/search-client.ts` | 搜索结果增强流水线 |
| `src/lib/search-selection.ts` | 正文选区校验与搜索查询规范化 |
| `src/lib/search-spotlight.ts` | 聚光灯参数处理 |
| `src/components/search.tsx` | 查询与 Chapter 范围 UI |
| `src/components/search-spotlight.tsx` | 搜索结果聚光灯 |
| `src/app/[lang]/docs/layout.tsx` | locale 页面树与导航布局 |
| `src/components/docs-reading-return.tsx` | 跨文档上下文恢复 |
| `src/lib/docs-reading-restore.ts` | 文档刷新滚动恢复 |
| `src/app/[lang]/docs-source/[...slug]/route.ts` | Markdown 源码端点 |
| `src/lib/layout.shared.tsx` | Fumadocs UI i18n |
| `src/components/guestbook.tsx` | Giscus locale 与主题同步 |
| `src/components/docs-community.tsx` | 文档评论区隔离 |
