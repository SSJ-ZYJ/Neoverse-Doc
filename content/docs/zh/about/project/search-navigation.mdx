...
> [!DETAILS-AI] 本章 AI 摘要
> 项目把搜索索引、页面树和源码端点静态生成，再在浏览器中增加 Chapter 范围筛选、搜索结果增强流水线、搜索聚光灯与跨文档阅读返回。中文搜索使用中英混合分词，并只为标题和小节标题增加拼音检索。Giscus 主题切换通过 `useLayoutEffect` 隐藏旧 iframe，并等待新主题 iframe 就绪后再显示。路由转场独立成 [专题](./transitions) 说明。
...
- 使用主题 URL 作为 Giscus key 重挂载 iframe，避免依赖没有完成回调的主题 `postMessage`；
- 通过 Shadow DOM 中 iframe 的 `load` 与 `loading` class 变化判断就绪，并以有限时长兜底避免外部网络异常时永久隐藏；
