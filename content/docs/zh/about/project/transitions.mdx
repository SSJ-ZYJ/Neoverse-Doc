...
路由转场由 `src/components/transition/` 下的六个文件协作完成：
...
`src/styles/transitions.css` 定义转场视觉规则。
...
| 文件 | 职责 |
| :--- | :--- |
| `src/components/transition/transition-types.ts` | 类型契约 |
| `src/components/transition/transition-policy.ts` | 路由到转场语义的映射 |
| `src/components/transition/transition-controller.ts` | DOM 克隆与几何计算 |
| `src/components/transition/transition-provider.tsx` | 转场状态与清理 |
| `src/components/transition/transition-layer.tsx` | inert 视觉层 |
| `src/components/transition/transition-link.tsx` | next/link 包装 |
| `src/components/transition/back-link.tsx` | 声明式返回链接 |
| `src/components/transition/content-particle-transition.ts` | docs 间 WebGL 粒子退场 |
| `src/styles/transitions.css` | 转场视觉规则 |
| `src/lib/motion-config.ts` | 转场时长与超时配置 |
