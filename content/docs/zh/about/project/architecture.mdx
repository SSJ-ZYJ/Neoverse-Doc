...
│   ├── generated/                # 生成的资源映射
│   ├── lib/                      # 内容源、搜索、i18n 与解析逻辑
│   └── styles/                   # Token、页面与组件样式
├── source.config.ts              # 内容 Schema 与 MDX 插件
...
`src/lib/` 保存可复用解析与配置逻辑，`scripts/` 只负责需要在开发或构建前批量完成的工作。生成产物分别进入 `public/mermaid/`、`src/generated/`、`.source/` 与 `out/`。
...
`source.config.ts` 通过 `defineDocs()` 声明内容目录、Frontmatter Schema 与 Markdown / MDX 插件。`fumadocs-mdx` 把内容编译到 `.source/`，再由 `src/lib/source.ts` 的 `loader()` 组合为页面树。
...
生产构建还会从同一内容源生成 `robots.txt`、只包含已发布页面的 `sitemap.xml`，以及统一的 PNG 社交分享图。草稿与辅助路由的收录边界在构建期确定，不依赖部署时服务端判断。
...
| 文件 | 职责 |
| :--- | :--- |
| `source.config.ts` | 内容 Schema、Remark / Rehype 与代码高亮配置 |
| `next.config.ts` | 生产静态导出、图片与开发响应配置 |
| `src/lib/source.ts` | Fumadocs 内容 loader 与图标映射 |
