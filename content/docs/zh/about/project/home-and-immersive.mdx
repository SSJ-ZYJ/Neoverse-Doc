...
`src/components/immersive-interaction-controller.tsx` 实现 HarmonyOS 风格光子场：沿指针路径连续生成小型圆点光子，响应式密度与平滑外散。
...
| 选择器常量 | 目标 | 行为 |
| :--- | :--- | :--- |
| `CONTROL_SELECTOR` | `.control-surface`、`.chapter-card`、`.mdx-doc-card`、`#nd-nav button` | 控件级粒子 |
| `SURFACE_SELECTOR` | `.surface-panel`、`.glass-codeblock`、`.markdown-alert`、`.markdown-details` | 表面级粒子 |
| `SIDEBAR_FOOTER_PARTICLE_HOST_SELECTOR` | 侧栏底部工具栏 | 整体作为粒子宿主 |
| `CODE_TABS_PARTICLE_HOST_SELECTOR` | 代码 Tabs | 共享一个粒子层 |

### 核心算法

- `burstConfig()`：根据 `pointerType`（touch / pen / mouse）、surface 类型、motion level（medium 折半）调整粒子数与发射距离；
...
`src/styles/immersive-interactions.css` 定义粒子视觉：
...
| `src/lib/home-sections.ts` | 首页章节与搜索标签数据源 |
| `src/styles/pages/home.css` | 首页样式与约束 |
| `src/components/immersive-interaction-controller.tsx` | 粒子交互控制器 |
| `src/components/immersive-scrollbar.tsx` | 视口滚动条与 TOC thumb |
...
| `src/styles/immersive-interactions.css` | 沉浸式交互样式 |
