...
> [!DETAILS-AI] 本章 AI 摘要
> 首页由 `HomePortal` 组织成 Hero、知识主入口、Chapter 线性浏览与共建四个层级；知识主入口消费 Learn / Explore / Reference Product Projection，Chapter 只保留为次级浏览方式。环境动效控制器提供共享动画时钟与帧率预算。沉浸式交互沿指针路径生成 HarmonyOS 风格光子场，TOC 滚动条以 4px thumb 连续追踪目录高亮，文档页渐进模糊与行内代码换行检测通过 `IntersectionObserver` 与 `MutationObserver` 实现按需测量。所有持续动画尊重 `prefers-reduced-motion`，标签页隐藏时暂停 RAF 与粒子层。

## 一、首页知识门户结构

`src/components/home/home-portal.tsx` 是首页入口组件，由 `AmbientMotionController` 包裹四个层级：
...
首页先建立项目身份，再回答“我现在想做什么”，然后提供完整章节的线性浏览与共建入口。Learn / Topics / Reference 是正式产品入口，Chapter 页面树只用于次级线性导航。
...
## 四、知识主入口与 Chapter 次级段

知识主入口段使用 `variant="knowledge"` 加速器背景承载三个正式产品入口。Chapter 段展示从 Fumadocs 页面树派生的完整章节入口，是首页的次级线性浏览区，不参与 Learn、Topics 或 Reference 的分类与排序。

### AiComputeBackdrop

`src/components/home/ai-compute-backdrop.tsx` 渲染结构化 SVG 动画作为知识主入口段背景：

- `variant="knowledge"` 用于知识主入口段，呈现知识网络视觉；
...
- 卡片内容（标题、描述）从 `home-sections.ts` 的页面树派生数据动态读取。

### home-sections.ts

`src/content/home-sections.ts` 从 Fumadocs `source.getPageTree(locale)` 提取 root folders 作为首页 Chapter 次级入口：
...
| 文件 | 职责 |
| :--- | :--- |
| `src/components/home/home-portal.tsx` | 首页知识门户入口 |
| `src/components/home/home-entry-grid.tsx` | Learn / Topics / Reference 主入口 |
