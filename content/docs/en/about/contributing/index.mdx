...
| :--- | :--- | :---: |
| `bun dev` | Generate Mermaid assets and start the Turbopack development server | Yes |
| `bun lint` | Run Biome Lint | No |
| `bun typecheck` | Generate route and content types; validate MDX, Frontmatter, and TypeScript | Yes |
| `bun run generate:mermaid` | Regenerate Mermaid static SVGs and the asset mapping | Yes |
| `bun run build` | Generate Mermaid assets and run the production static build | Yes |
...
`author`, `contributor`, and `contributors` accept a single string or an array of strings; `draft` and `todoProgress` both default to `false`. Do not write custom fields that are not declared by the Schema.
...
| Mermaid content or count changes | `bun run generate:mermaid`, and check the generated assets |
