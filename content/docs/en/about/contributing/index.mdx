...
<Files>
  <Folder name="Neoverse-Doc" defaultOpen>
    ...
    <Folder name="scripts" disabled title="Build-time generation scripts" />
    <Folder name="src" defaultOpen>
      ...
      <Folder name="generated" disabled title="Generated asset mappings" />
      <Folder name="lib" disabled title="Configuration, content sources, and utility functions" />
    </Folder>
...
The current repository recommends:

```text
<type>(<scope>): <summary>
```

Common types include `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`, `ci`, and `revert`. For example:

```text
docs(about): rewrite the contributing guide
fix(search): fix the chapter filter state
feat(mdx): add a learning path component
```

Keep the summary short; when additional background is needed, add the body after a blank line. PR titles can follow the same format.
