...
Open `http://localhost:3001` in a browser to preview. Do not add, remove, upgrade, or replace dependencies without discussion; the current versions are based on `package.json`.
...
| `bun run report:content` | Print Content IR Metadata Coverage and Content Health; append `-- --verbose` or `-- --taxonomy` as needed | No |
| `bun run check:content` | Validate Schema, lifecycle, relations, translation drift, and Mermaid assets; low Coverage never fails | Yes |
...
`id` is the required stable content identity: when creating a page, take the value matching its final path (e.g. `ch1/your-page-name`); the zh and en versions must declare the same `id`. After that, never change it when moving the file or adjusting the URL. The template's `type`, `topics`, `tracks`, and `difficulty` are the minimum recommended Metadata for a new page, but they remain optional and can be added incrementally. `estimatedMinutes`, `prerequisites`, `related`, `lastReviewed`, `author`, `contributor`, `contributors`, `status`, and `todoProgress` are also optional; `status` defaults to `stable`. Only record `lastReviewed` after a human technical review; add the matching `reviewedRevision` to a translation after confirming it is synchronized. Do not write custom fields that are not declared by the Schema.

Run `bun run report:content` to see current Coverage and maintenance summaries. Missing Metadata produces authoring warnings only and does not block CI. Use `bun run report:content -- --verbose` for complete file-level diagnostics, or `bun run report:content -- --taxonomy` for legal Type, Topic, Track, and Difficulty IDs. These lists are generated from the Taxonomy Registry; do not maintain a second list in the documentation.
...
See [Syntax and Component Reference](./contributing/syntax-example) for the complete syntax and rendering examples.
