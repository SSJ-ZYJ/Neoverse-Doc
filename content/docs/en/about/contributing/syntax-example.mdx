...
After modifying Mermaid content or the diagram count, run `bun run generate:mermaid` or `bun run build`, and check the generated SVGs and `src/generated/mermaid-assets.ts`.
