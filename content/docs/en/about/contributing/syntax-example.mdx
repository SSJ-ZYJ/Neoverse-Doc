...
| `type` | No | Content type; legal IDs are available from `bun run report:content -- --taxonomy` |
| `topics` | No | Topic ID array; legal IDs are available from `bun run report:content -- --taxonomy` |
| `tracks` | No | Learning-track ID array; legal IDs are available from `bun run report:content -- --taxonomy` |
| `difficulty` | No | Difficulty ID; legal IDs are available from `bun run report:content -- --taxonomy` |
| `estimatedMinutes` | No | Estimated learning time in positive integer minutes |
| `prerequisites` | No | Array of prerequisite Stable Content IDs |
| `related` | No | Array of related Stable Content IDs |
...
| `status` | No | Defaults to `stable`; accepts `draft`, `review`, `stable`, or `deprecated` |
| `lastReviewed` | No | The latest technical-validity confirmation date, in `YYYY-MM-DD` form |
| `replacement` | No | A replacement Stable Content ID for `deprecated` pages, such as `docs:ch1/new-page` |
| `reviewedRevision` | No | The source revision most recently confirmed by a translation (SHA-256) |
...
For a new page, `type`, `topics`, `tracks`, and `difficulty` are recommended first additions; add `estimatedMinutes`, `prerequisites`, `related`, and `lastReviewed` incrementally as the content warrants. Run `bun run report:content` for Coverage and `bun run report:content -- --taxonomy` for legal Registry IDs. Low Coverage is an authoring warning only and does not block CI.
...
This site enhances standard GFM task items into interactive compatibility Checklists, with state saved in the current browser. To show Checklist progress—or the progress of explicit Learning Tasks—at the top of the page, set `todoProgress: true` in the Frontmatter.

### Explicit Learning Tasks

Ordinary GFM Checkboxes are Checklists; only content explicitly written with `Lab` and `Task` enters persisted learning progress. `Lab id` and `Task id` are stable identities, so do not use display text that may change as an id.
...
`Task` must be nested inside a `Lab`. The tuple `contentId + labId + taskId` determines task identity; changing display text, the title, or the page URL does not change existing state.
