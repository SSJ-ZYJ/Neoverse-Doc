...
> The homepage is organized by `HomePortal` into Hero, primary knowledge entries, secondary Chapter browsing, and community layers. The primary entries consume the Learn / Explore / Reference Product Projections, while Chapters remain a secondary linear-browsing option. An ambient motion controller provides a shared animation clock and frame budget. The immersive interaction generates a HarmonyOS-style photon field along the pointer path, the TOC scrollbar continuously tracks the directory highlight with a 4px thumb, and progressive blur on documentation pages plus inline code wrapping detection use `IntersectionObserver` and `MutationObserver` for on-demand measurement. All continuous animations respect `prefers-reduced-motion`, and RAF and the particle layer pause when the tab is hidden.

## 1. Homepage knowledge portal structure

`src/components/home/home-portal.tsx` is the homepage entrance component. It wraps four layers of content with `AmbientMotionController`:
...
│   ├── Primary knowledge-entry section
│   │   ├── AiComputeBackdrop variant="knowledge" (AI accelerator backdrop)
│   │   ├── HomeEntryGrid (Learn / Topics / Reference)
│   │   └── Counts derived from Product Projections
│   ├── Secondary Chapter section
...
The homepage first establishes project identity, then answers “what do I want to do now?”, followed by complete Chapter browsing and contribution links. Learn, Topics, and Reference are formal product entries; the Chapter page tree only supplies the secondary linear navigation.
...
## 4. Primary knowledge entries and secondary Chapter section

The primary knowledge-entry section uses the `variant="knowledge"` accelerator backdrop for the three formal product entries. The Chapter section shows the complete Chapter tree derived from the page tree. It is a secondary linear-browsing area and does not classify or order Learn, Topics, or Reference.

### AiComputeBackdrop

`src/components/home/ai-compute-backdrop.tsx` renders a structured SVG animation as the primary knowledge-entry section's backdrop:

- `variant="knowledge"` is used for the primary entry section, presenting a knowledge-network visual;
...
- Card content (title and description) is derived dynamically from the page-tree data in `home-sections.ts`.

### home-sections.ts

`src/content/home-sections.ts` extracts root folders from Fumadocs `source.getPageTree(locale)` as secondary homepage Chapter entries:
...
Fumadocs-generated elements that cannot carry the contract (nav / sidebar controls, code tabs, the mobile title bar, the sidebar footer) and pipeline outputs (plain `blockquote`s, table scroll wrappers) are translated to kinds by the adapter layer in `registry.ts`; no other runtime module learns Fumadocs DOM.
...
| File | Responsibility |
| :--- | :--- |
| `src/components/home/home-portal.tsx` | Homepage knowledge portal entrance |
| `src/components/home/home-entry-grid.tsx` | Primary Learn / Topics / Reference entries |
