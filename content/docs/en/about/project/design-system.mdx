...
`src/app/globals.css` is the aggregation entry point; actual rules are split by responsibility into `src/styles/`:

```text
src/styles/
├── theme.css              # Tailwind / Fumadocs base mapping
├── tokens.css             # colors, surfaces, radii, shadows, spacing, and motion
├── motion.css             # entrance animations and reduced-motion
├── glass.css              # glass surfaces and controls
├── fumadocs-glass.css     # Fumadocs component overrides
├── typography.css         # body text, code, blockquotes, and callouts
├── transitions.css        # page transitions
├── mermaid.css            # diagram theme and nodes
├── loading.css            # route loading
├── giscus.css             # comment styles
├── mdx-files.css          # file hierarchy styles
├── search-spotlight.css   # search spotlight
├── immersive-interactions.css # immersive interactions
├── a11y.css               # accessibility and transparency fallbacks
└── pages/                 # page-level structure
    ├── home.css
    └── special.css
```

Each file takes a single responsibility; visual tweaks prefer replacing Tokens or existing surfaces over stacking a second glass, Glow, or shadow system.

## 2. Semantic Token System

`src/styles/tokens.css` is the single source of truth for all visual decisions, providing dual variable sets for light (`:root, .light`) and dark (`.dark`).
...
`src/styles/glass.css` implements the unified glass visual language.
...
`src/styles/theme.css` is the integration point for Tailwind v4 and Fumadocs:
...
Mermaid static SVGs adapt to the theme via CSS variables and project class names, so there is no need to generate two diagram sets for light and dark. `src/styles/mermaid.css` provides complete semantic color slots; Mermaid nodes reference variables through class names.

## 6. Motion Tiers and Base Animations

`src/styles/motion.css` defines the unified entrance animations and reduced-motion handling.
...
`src/components/motion-preferences-provider.tsx` exposes `effectiveLevel`, `effectiveExperimental`, `systemReducedMotion`, and `experimentalMotionSupported` through Context:
...
`src/styles/a11y.css` handles three fallback layers so content stays readable when visual enhancements are unavailable.
...
`src/styles/typography.css` defines typography rules for body text, code, blockquotes, and callouts:
...
| File | Responsibility |
| :--- | :--- |
| `src/styles/tokens.css` | Dual semantic Token variable sets |
| `src/styles/theme.css` | Tailwind / Fumadocs mapping |
| `src/styles/glass.css` | Glass trio structure and business surfaces |
| `src/styles/motion.css` | Entrance animations and three motion tiers |
| `src/styles/a11y.css` | Three accessibility fallback layers |
| `src/styles/typography.css` | Body text, code, blockquotes, and callouts |
| `src/styles/fumadocs-glass.css` | Fumadocs component overrides |
| `src/components/motion-preferences-provider.tsx` | Motion preferences Context |
| `src/lib/motion-preferences.ts` | Motion preference parsing and bootstrap |
| `src/lib/motion-config.ts` | Centralized motion configuration |
| `src/lib/experimental-motion-support.ts` | Experimental motion capability detection |
