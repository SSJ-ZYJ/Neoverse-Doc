...
- **Search** intentionally stays on the Fumadocs source pipeline (indexes need tokenized structuredData; putting it into the IR would turn it into a body dump) — the one declared content consumer outside the IR. This phase adds neither a taxonomy Filter nor custom index fields; index generation remains build-time and Search UI remains a separate client consumer.
