...
| `status` | `draft` / `review` / `stable` / `deprecated`, default `stable` | The single content lifecycle; `draft` enables the soft gate |
| `lastReviewed` | `YYYY-MM-DD`, optional | The latest date on which technical validity was confirmed; never derived from Git mtime |
| `replacement` | `docs:<id>`, optional | A Stable Content ID for the replacement of a deprecated page |
| `reviewedRevision` | SHA-256, optional on translations | The source revision most recently confirmed by a translation |
...
The translation-key duty is explicitly carried by "the zh and en versions of the same content declare the same `id`"; lifecycle is expressed only by `status`, so `draft` is not retained as a competing field. Existing `draft: true` pages have been migrated to `status: draft`, and the old field is rejected by the Schema. Review dates are manually confirmed and never replaced by Git history.

### Content Maintenance Model

The lifecycle has four states: `draft` means the page is still being written and uses the existing soft gate; `review` means the body is readable but being checked; `stable` means the content is currently considered stable; and `deprecated` means the page remains reachable but is no longer the current recommendation. A deprecated page may point to a replacement with `replacement: docs:<id>`; the build verifies that the target exists and is not self-referential.

Freshness Policy comes from one centralized maintenance policy under `src/content/taxonomy/`, rather than being repeated in components. The current intervals are 90 days for `fast-moving`, 365 days for `normal`, and 730 days for `stable`; when multiple type or Topic policies match, the shortest interval wins. A published page with no `lastReviewed` date or an expired date produces a warning in `bun run check:content`, not a build failure; `draft` and `deprecated` pages do not add a duplicate freshness alarm.

Translation Drift uses the default `zh` locale as the source and pairs pages by Stable Content ID. The source body and reader-facing metadata produce an automatic SHA-256 `contentRevision`; a translation stores the source revision it last confirmed in `reviewedRevision`. Equal values are `up-to-date`, an absent locale page is `missing`, and a missing or mismatched revision is `outdated`. Existing translations are not auto-filled with confirmation values when this model is introduced.
...
| `bun run check:content` | Production gate (auto-run by `prebuild`) | Derive IR → validate content and emit maintenance report → hash-check mermaid assets; never imports Puppeteer |
| `bun run report:content` | Authoring DX report | Derive IR → print Metadata Coverage, Content Health, and optional Taxonomy; never touches Mermaid assets; append `-- --verbose` for complete diagnostics |
...
- **Content validation** (ID uniqueness, translation pairing, reference existence, duplicates, self-references, cross-locale relation conflicts, and prerequisite cycles) runs on the IR; lifecycle relations, replacement targets, and invalid date semantics are Errors, while Freshness and translation `missing` / `outdated` results are Warnings;
- **Translation Report** prints `missing`, `outdated`, or `up-to-date` for every locale paired by Stable Content ID; warnings do not change the successful exit code of `check:content`;
- **Metadata Coverage / Authoring Diagnostics** consume Content IR directly and count `type`, `topics`, `tracks`, `difficulty`, `estimatedMinutes`, `prerequisites`, `related`, `status`, and `lastReviewed` per locale page variant; missing optional fields produce compact warnings only, never Required Schema or CI Errors. Run `bun run report:content -- --taxonomy` to discover legal Taxonomy IDs;
...
## 9. Lifecycle States and Draft Gating

`status: draft` does not remove an article from the static page tree. The page still participates in the build, but the body initially starts in an `inert` and inaccessible state, while `DocsDraftControls` shows the draft notice, a previous or home page entry, and an explicit preview button. `review` and `deprecated` pages remain readable and show a compact lifecycle notice beside the title; a deprecated page also links to its declared replacement when available.

Draft soft gating signals that content is not stable yet; it is not a permission system. No sensitive information should ever enter the generated static files. The old boolean `draft` form is migration history only and must not be used in new pages.
