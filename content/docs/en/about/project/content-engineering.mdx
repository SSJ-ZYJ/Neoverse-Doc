...
Enum, format, and numeric constraints are enforced by zod at compile time; cross-page references (ID uniqueness, reference existence, self-reference guard) are validated by `scripts/check-content.ts` against the compiled content manifest (`bun run check:content`, wired into `prebuild`). References follow locale-loose semantics — a target existing in any language is valid, so translation gaps never break content relations.

`translationKey`, `status`, and `lastReviewed` are deliberately absent: the translation-key duty is already carried by directory-based locale pairing and locale-independent derived IDs; publishing state already exists as `draft`; review dates treat git history as the source of truth. Existing pages are annotated incrementally — no batch migration.
