...
│   └── lib/                      # Low-coupling i18n, SEO, and site configuration
...
Dependency directions between the nine top-level directories are guarded automatically by `scripts/check-architecture.ts` (`bun run check:architecture`, also the first step of `prebuild`). The script scans every TypeScript import under `src/` (including dynamic `import(...)`) and flags layer violations against this matrix:
...
`lib` and `dictionaries` form the site-glue cluster (locale definitions and UI dictionaries reference each other) — the only intentional bidirectional pair in the matrix. `ui` must stay CSS-only; any `.ts` / `.tsx` file there fails the check immediately.

Two additional rules:

- **Barrel rule**: cross-boundary consumers of a feature must go through its `index.ts` public entry; importing an internal file under `features/A/` directly is rejected, while imports inside the feature itself are unrestricted. Public entries only expose capabilities genuinely needed across boundaries — no whole-directory re-exports.
- **Exception list**: legitimate bridges (for example, a Fumadocs adapter writing state into the navigation runtime) are registered centrally in the script's `EXCEPTIONS` constant, each with a documented reason, instead of loosening the whole rule; stale exceptions trigger a warning to clean up.

New top-level directories must be registered in the `ALLOWED` matrix, otherwise the check fails as an "unknown layer". The full decision record lives in `docs/adr/0001-architecture-boundary-check.md` in the repository.
...
1. the title, description, authors, and contributors from the Frontmatter;
...
flowchart TD
    ...
    A --> J[Mermaid generation script]
    J --> K[SVG and asset map]
    K --> G

    F --> L[Typed Content Manifest]
    L --> M[Static search index and sitemap]
    M --> H
```

`src/content/generated/manifest.ts` derives a read-only content list directly from the compiled Source. Stable IDs use `docs:${slugs.join('/')}`, so localized pages at the same path share an ID. The Manifest retains draft flags and consumers such as sitemap perform their own filtering; there is no second hand-maintained content source. The production build also generates `robots.txt`, a published-only `sitemap.xml`, and a unified PNG social share image.

Different commands cover different verification scopes:

| Command | Pipeline role |
| :--- | :--- |
| `bun dev` | Generates Mermaid first, then starts the Turbopack development server |
...
| `bun run generate:mermaid` | Updates the static SVGs and asset map |
| `bun run build` | Runs the architecture check, Mermaid generation, and the Next.js production build |
...
| `src/features/` | Six mature feature boundaries |
...
| `scripts/generate-mermaid-assets.ts` | Mermaid static asset generation |
