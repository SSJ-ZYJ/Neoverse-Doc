...
│   ├── components/
│   │   ├── home/                 # Home page content portal
│   │   ├── mdx/                  # MDX rendering components
│   │   └── transition/           # Route transitions
│   ├── dictionaries/             # Project UI dictionaries
│   ├── generated/                # Generated asset maps
│   ├── lib/                      # Content source, search, i18n, and parsing logic
│   └── styles/                   # Tokens, page, and component styles
├── source.config.ts              # Content Schema and MDX plugins
...
`src/lib/` holds reusable parsing and configuration logic, while `scripts/` only handles work that must be completed in bulk before development or builds. Generated artifacts go to `public/mermaid/`, `src/generated/`, `.source/`, and `out/` respectively.
...
`source.config.ts` declares the content directory, Frontmatter Schema, and Markdown / MDX plugins via `defineDocs()`. `fumadocs-mdx` compiles content into `.source/`, and `loader()` in `src/lib/source.ts` assembles it into the page tree.
...
flowchart TD
    ...
    F --> L[Static search index]
    L --> H
```

The production build also generates `robots.txt`, a `sitemap.xml` containing only published pages, and a unified PNG social share image from the same content source. The inclusion boundaries for drafts and auxiliary routes are decided at build time, not by server-side detection at deployment.
...
| `bun run build` | Runs Mermaid generation and the Next.js production build |
...
| File | Responsibility |
| :--- | :--- |
| `source.config.ts` | Content Schema, Remark / Rehype, and code highlighting configuration |
| `next.config.ts` | Production static export, image, and development response configuration |
| `src/lib/source.ts` | Fumadocs content loader and icon mapping |
