...
New top-level directories must be registered in the `ALLOWED` matrix, otherwise the check fails as an "unknown layer". The full decision records live in `docs/adr/0001-architecture-boundary-check.md` and `docs/adr/0005-strict-layer-dag.md` in the repository.
