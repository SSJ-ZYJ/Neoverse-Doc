...
│   ├── topics/
│   │   ├── page.tsx              # Topic entry
│   │   └── [topic]/page.tsx      # Topic content projection
│   └── reference/page.tsx        # Reference entry
├── docs/
...
- `/topics` and `/topics/[topic]` reuse the home layout, with content derived from the Topic Registry and Explore Projection;
- `/reference` reuses the home layout and exposes only content selected by the Reference Projection;
...
| `src/features/` | Nine mature feature boundaries (each with a public entry + the cross-feature allowlist) |
