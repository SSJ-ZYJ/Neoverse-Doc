...
> If you want commits to be associated with your GitHub account, use an email verified in your GitHub account (recommended) or the `noreply` email provided by GitHub. The commit identity and login credentials are two different things; before changing anything, you can check the locally configured email with `git config --global --get user.email`.
...
If the current branch is a direct ancestor of the branch being merged, Git may perform a **fast-forward**: it only moves the current branch reference and creates no merge commit. If both branches have commits after their common ancestor, Git usually performs a **three-way merge** based on the common ancestor and may create a merge commit; files that cannot be resolved automatically cause conflicts (see the next section). Whether to force a merge commit can follow the project convention with `--no-ff`.
...
**Three-way merge: both branches have commits after their common ancestor**
...
The two branches advance separately after the common ancestor B; Git does a three-way merge based on the common ancestor and produces merge commit M.
...
| 13-A | GitHub account authorization, branch protection, PR, Issue, Releases | [1.16 GitHub and CNB](/en/docs/ch1/1.16-GitHub-CNB) |
| 13-B | Contributing to large open source projects | [1.16 GitHub and CNB](/en/docs/ch1/1.16-GitHub-CNB) |
