...
> Docker Desktop uses the WSL 2 backend by default on Windows; some versions and scenarios also support the Hyper-V backend. If using WSL 2, install WSL first per [1.14 Environment Setup](/en/docs/ch1/1.14-Environment-Setup), and confirm that WSL integration is enabled in Docker Desktop.
