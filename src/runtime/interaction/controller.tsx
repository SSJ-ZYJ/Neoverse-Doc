...
function readParticleGeometry(target: HTMLElement): ParticleGeometry {
  ...
  if (!target.matches(FUMADOCS_SIDEBAR_FOOTER_SELECTOR)) {
    return {
...
export function ImmersiveInteractionController() {
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      ...
      const target =
        source?.closest<HTMLElement>(FUMADOCS_SIDEBAR_FOOTER_SELECTOR) ??
        source?.closest<HTMLElement>(FUMADOCS_CODE_TABS_SELECTOR) ??
        source?.closest<HTMLElement>(FUMADOCS_MOBILE_TITLE_SELECTOR) ??
        source?.closest<HTMLElement>(INTERACTIVE_SELECTOR);
      if (!target || target.matches(':disabled, [aria-disabled="true"]')) return;
      if (target.matches(FUMADOCS_SIDEBAR_DOCUMENT_GROUP_TRIGGER_SELECTOR)) return;
      ...
      if (
        target.matches(FUMADOCS_SIDEBAR_FOOTER_SELECTOR) &&
