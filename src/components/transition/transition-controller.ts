...
function findSourceNode(): HTMLElement | null {
  const candidates = document.querySelectorAll<HTMLElement>('main, #nd-docs-layout');
  return Array.from(candidates).find((node) => !node.closest('#nd-transition-layer')) ?? null;
}
