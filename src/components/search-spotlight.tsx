...
import { getSpotlightScrollDelta, SEARCH_SPOTLIGHT_PARAM } from '@/lib/search-spotlight';
...
export function SearchSpotlight() {
  ...
  useEffect(() => {
    ...
    const show = () => {
      const currentUrl = new URL(window.location.href);
      root = document.querySelector<HTMLElement>(DOCS_PAGE_SELECTOR);
      if (
        currentUrl.pathname !== targetPathname ||
        currentUrl.searchParams.get(SEARCH_SPOTLIGHT_PARAM) !== target ||
        !root ||
        document.documentElement.querySelector(ROUTE_TRANSITION_SELECTOR)
      ) {
