// Canvas UI Particle Scroll standalone source, adapted to use capability,
// WebGL2, and reduced-motion fallbacks in Neoverse Docs.
// Canvas UI Particle Scroll 独立源码，并为 Neoverse Docs 接入能力检测、
// WebGL2 失败与减少动态效果回退。
'use client';
...
function requireWebGlResource<T>(resource: T | null, label: string): T {
  if (!resource) throw new Error(`Unable to create ParticleScroll ${label}.`);
  return resource;
}
...
export function createParticleScroll(
  ...
  const webGlContext = output.getContext('webgl2', {
    alpha: true,
    depth: false,
    stencil: false,
    antialias: false,
  ...
  const gl: WebGL2RenderingContext = webGlContext;
  ...
  function compile(type: number, text: string): WebGLShader {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      throw new Error(`ParticleScroll shader error: ${message}`);
    }
    return shader;
  }
  ...
  function link(vertText: string, fragText: string) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      ...
      throw new Error(`ParticleScroll program error: ${message}`);
    }
  ...
  const quadVao = requireWebGlResource(gl.createVertexArray(), 'quad vertex array');
  gl.bindVertexArray(quadVao);
  const quad = requireWebGlResource(gl.createBuffer(), 'quad buffer');
  gl.bindBuffer(gl.ARRAY_BUFFER, quad);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, 1, -1, -1, 1, 1, 1]), gl.STATIC_DRAW);
  gl.enableVertexAttribArray(0);
  gl.vertexAttribPointer(0, 2, gl.FLOAT, false, 0, 0);
  const pointVao = requireWebGlResource(gl.createVertexArray(), 'point vertex array');

  const contentTexture = requireWebGlResource(gl.createTexture(), 'content texture');
  ...
  const rowTex = requireWebGlResource(gl.createTexture(), 'row texture');
    ...
    setOptions(next) {
      if (
        !Object.entries(next).some(
          ([key, value]) => config[key as keyof ParticleScrollOptions] !== value,
        )
      )
        return;
...
export function ParticleScroll({ children, className, style, ...options }: ParticleScrollProps) {
  ...
  return (
    <div
      className={className}
      data-particle-scroll-native={native ? '' : undefined}
